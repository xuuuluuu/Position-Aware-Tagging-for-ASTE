...
class TriextractNeuralBuilder(NeuralBuilder):
	def __init__(self, gnp, voc_size, label_size, char2id, chars, char_emb_size, charlstm_hidden_dim, lstm_hidden_size, dropout, pos_emb_size, token_emb_size, bert_emb):
		...
		self.bert_emb = bert_emb + bert_emb
		...
		self.linear_polar = nn.Linear(lstm_hidden_size * 3, 3).to(NetworkConfig.DEVICE)
	...
	def build_nn_graph(self, instance):
		...
		segment_out = lstm_out.squeeze(0)
		linear_output = self.linear(segment_out).squeeze(0)
		#score of each node
		...
		for i in range(instance_len):
			for j in range(i, instance_len):
				if i==0 and j+1 == instance_len:
					segment_emb = torch.cat([segment_out[j][0:self.lstm_hidden_size], segment_out[i][self.lstm_hidden_size:]], 0)
				elif i==0 and j+1 < instance_len:
					segment_emb = torch.cat([segment_out[j][0:self.lstm_hidden_size], segment_out[i][self.lstm_hidden_size:] - segment_out[j+1][self.lstm_hidden_size:]], 0)
				elif  i>0 and j+1 == instance_len:
					segment_emb = torch.cat([segment_out[j][0:self.lstm_hidden_size] - segment_out[i-1][0:self.lstm_hidden_size], segment_out[i][self.lstm_hidden_size:]], 0)
				else:
					segment_emb = torch.cat([segment_out[j][0:self.lstm_hidden_size] - segment_out[i-1][0:self.lstm_hidden_size] , segment_out[i][self.lstm_hidden_size:] - segment_out[j+1][self.lstm_hidden_size:]], 0)
		...
		return torch.cat([linear_output, zero_col], 0), span_score, polar_score, offset_score, segment_out, seg_embs
	...
	def get_nn_score(self, network, parent_k):
		...
		if node_type == 0 or node_type == 2:
			...
			if label_str == 'B' or label_str == 'S':
				...
				pos_score = offset_score[pos_target][0]#self.pos_embed_linear(self.pos_embed(pos_target))[0][0]
				# #original
				base_score = base_score + span_score[opinion_boundary][0] + pos_score + self.linear_polar(torch.cat((seg_embs[opinion_boundary], lstm_out[pos][lstm_hidden_size:]), 0))[polarity]#offset_score[pos_target]
				# ablation
				# base_score = base_score + pos_score + self.linear_polar(torch.cat((seg_embs[opinion_boundary], lstm_out[pos][lstm_hidden_size:]), 0))[polarity]#offset_score[pos_target]
